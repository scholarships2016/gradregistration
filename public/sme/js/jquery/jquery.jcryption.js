...
(function($) {
				...
				if(base.options.beforeEncryption()) {
					$.jCryption.getKeys(base.options.getKeysURL, function(keys) {
						$.jCryption.encrypt(base.$el.serialize(), keys,function(encrypted) {
							...
							var $thisForm = $(base.$el).find(base.options.formFieldSelector);
                                                        $thisForm.attr("disabled",true);
                                                        var tmpElement = $("input[name='"+base.options.postVariable+"']");
                                                        
                                                        if(tmpElement.length>0){
                                                            tmpElement.attr("disabled",false);
                                                            tmpElement.val(encrypted);
                                                        }else{
                                                            $thisForm.end().append($encryptedElement);
                                                        }

                                                        if(base.options.afterEncryption()){
                                                        }else{
                                                            $thisForm.submit();
                                                        }
						});
	...
	$.jCryption.defaultOptions = {
		...
		formFieldSelector:":input",
                afterEncryption:function(){return true}
	};
