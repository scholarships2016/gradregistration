    ...
    public function searchByCriteriaGroup($curriculum_id = null, $curr_act_id = null, $criteria = null, $faculty_id = null, $degree_id = null, $status = null, $is_approve = null, $program_id = null, $inTime = true, $paging = false, $academic_year = null, $semester = null, $round_no = null,$program_type = null)
    {
        ...
        try {
            ...
            $cur = Curriculum::leftJoin('curriculum_program', 'curriculum.curriculum_id', '=', 'curriculum_program.curriculum_id')
                ...
                ->distinct()
                ->select([DB::raw('curriculum.curriculum_id,curriculum_activity.curr_act_id ,apply_method ,responsible_person  ,additional_detail  ,apply_fee ,additional_question  ,mailing_address ,document_file  ,comm_appr_name  ,comm_appr_no ,comm_appr_date  ,contact_tel ,is_approve ,expected_amount ,curriculum.status  , program_id ,tbl_program_type.program_type_id  ,curr_act_id ,apply_setting.apply_setting_id ,exam_schedule ,announce_exam_date ,announce_admission_date ,orientation_date ,orientation_location ,tbl_project.project_id ,project_name ,project_name_en ,curr_sub_major_id ,tbl_sub_major.sub_major_id ,sub_major_name ,sub_major_name_en  ,prog_type_name ,prog_type_name_en ,cond_id ,degree_level_name ,office_time ,programsystem ,studyprogramsystem ,calendar ,coursecodeno ,degree ,depcode ,majorcode ,noyear ,minperiod ,maxperiod ,credittot   ,language ,beginacadyear ,beginsemester ,lastacadyear ,lastsemester ,stopacadyear ,stopsemester ,thai ,english ,degreethai ,degreeenglish ,apply_setting.status apply_status,usercode ,updatedate ,changestame , semester ,academic_year ,round_no ,start_date ,end_date ,is_active  ,tbl_major.major_id ,major_name ,major_name_en ,tbl_department.department_id ,tbl_degree.degree_id ,degree_name ,degree_name_en ,tbl_faculty.faculty_id ,faculty_name, faculty_eng ,fac_sort ,faculty_full ,thai,coursecodeno,sub_major_name,tbl_sub_major.sub_major_id, department_name ,department_name_en  ')])
                ->orderBy('curriculum.curriculum_id');
