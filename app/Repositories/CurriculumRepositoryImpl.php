        ...
        try {
            $query = DB::table('curriculum as curr')
                ->select("curr.curriculum_id as id",
                    DB::raw("concat(deg.degree_name,' ,คณะ',fac.faculty_name,' ,สาขาวิชา',mj.major_name,' แขนงวิชา [',ifnull(group_concat(sub_mj.sub_major_name SEPARATOR ','),'-'),']',' รหัสวิชา [',ifnull(group_concat(curr_prog.program_id SEPARATOR ','),'-'),']') as text")
                )->leftJoin("curriculum_program as curr_prog", function ($join) {
                    $join->on("curr_prog.curriculum_id", "=", "curr.curriculum_id");
