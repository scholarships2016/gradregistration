...
class ApplyController extends Controller {
    ...
    public function getRegisterCourse(Request $request = null) {
        ...
        $curDiss = $this->CurriculumRepo->searchByCriteriaGroup(null, null, $request->searchs, $request->faculty_id, $request->degree_id, 1, 4, $request->program_id, true, true, null, null, null,  $request->program_type_id, $request->all(),session('user_id'),false);
    ...
    public function registerDetailForapply($id) {
        ...
        $curDiss = $this->CurriculumRepo->searchByCriteriaGroup(null, $id[0], null, null, null, null, null, null, true, false, null, null, null, $id[1],null,session('user_id'),true);
                ...
                foreach ($currs as $curr) {
                    ...
                    foreach ($apps as $app) {
                        ...
                        Mail::send('email.confirm-apply', $data, function($message)use ($app) {
                            $message->to($app->stu_email, $app->stu_first_name)->subject('Confirmation Result ');
                        });
                        Controller::WLog('Register-Confirmation [' . $app->stu_email . ']', 'Register-Confirmation', null);
                        session()->flash('successMsg', Lang::get('resource.lbSuccess'));
                      }else{
                        Controller::WLog('Register-Confirmation [' . $app->stu_email . ']', 'Register-Confirmation',  "Invalid Email Address ({$app->stu_email})");
                        session()->flash('errorMsg', Lang::get('resource.lbError'));
                      }
